...
<main>
  <div id="claimedWithSuccess" uk-modal>
    <div
      class="uk-modal-dialog uk-modal-body uk-text-center"
      style="background-image: url('/images/confetti.gif')"
    >
      <div class="uk-section">
        <h2 class="uk-modal-title">
          <span class="success-icon" uk-icon="icon: check; ratio: 2" />
        </h2>
        <p class="uk-text-small">{$_('make_whole.modal_success.title')}</p>
        {@html $_('make_whole.modal_success.amount_for_account', {
          values: {
            coins: selected && printCoins(selected.coins.value),
            account: selected ? selected.account : '',
          },
        })}
      ...
      <p class="uk-text-center">
        <button
          ...
          type="button">{$_('make_whole.modal_success.ok_btn')}</button
        >
      </p>
    </div>
  </div>
  ...
  <div id="claimError" uk-modal>
    <div class="uk-modal-dialog uk-modal-body uk-text-center">
      ...
      <p class="uk-text-center">
        <button
          class="uk-button uk-button-large uk-button-primary uk-margin-right uk-modal-close"
          type="button">{$_('make_whole.modal_error.ok_btn')}</button
        >
      </p>
    </div>
  </div>
          ...
          {#each credits as credit}
            <tr>
              ...
              <td class="uk-text-center" style="width: 200px;">
                {#if credit.claimed}
                  <span class="uk-text-success" uk-icon="icon: check; ratio: 1; color: green" />
                {:else}
                  <button
                    ...
                    >{isProcessing
                      ? $_('make_whole.claim_btn.await')
                      : $_('make_whole.claim_btn.claim')}</button
                  >
                {/if}
              </td>
            </tr>
